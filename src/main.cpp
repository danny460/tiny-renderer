...
void triangle(Vec3f *pts, Vec2f *texture_coords, float* zbuffer, TGAImage &image, TGAImage &texture, TGAColor c){
    ...
    for(P.x = minX ; P.x <= maxX; P.x++){
        for(P.y = minY; P.y <= maxY; P.y++){
            ...
            P_texture.x = (texture_coords[0].x * bc.x + texture_coords[1].x * bc.y+ texture_coords[2].x * bc.z ) * texture.get_width() + .5f;
            P_texture.y = (texture_coords[0].y * bc.x + texture_coords[1].y * bc.y+ texture_coords[2].y * bc.z ) * texture.get_height() + .5f;
            TGAColor color = texture.get(P_texture.x, P_texture.y);
            if(P.z > zbuffer[idx]){
                zbuffer[idx] = P.z;
...
int main(){
    ...
    for (int i=0; i<model.nfaces(); i++) { 
        ...
        if(intensity > 0){
            triangle(screen_coords, texture_coords, zbuffer, image, diffuse_texture, TGAColor(intensity*255, intensity*255, intensity*255, 255));
        }
    }
    // wireframe
    // for (int i=0; i<model.nfaces(); i++) { 
